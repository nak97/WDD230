...
async function tomorrow(){
  ...
  console.log(data);
  document.querySelector('#temp-tomorrow').innerHTML = `expected temperature ${data.list[5].main.temp}`
}
...
async function currentWeather(){
  ...
  console.log(data);
    ...
    document.querySelector('#current-temp').innerHTML = `current temperature ${data.main.temp}`;
...
document.querySelector("#x ").onclick = function(){  
  document.querySelector(".max-temp").style.display = "none"}
